...
if(player1Turn){
    ...
    builder.setMessage("Player X got first turn");
    builder.show();
}
else {
    AlertDialog.Builder builder;
    ...
    builder.setCancelable(true);
    builder.setPositiveButton("OK", (dialog, which) -> dialog.dismiss());
    builder.setMessage("Player O got first turn");
    builder.show();
}
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                int resID = 
                        getResources().getIdentifier(buttonID, "id", getPackageName());
                buttons[i][j] = findViewById(resID);
                buttons[i][j].setOnClickListener(this);
            }
        }

        Button buttonReset = findViewById(R.id.btn1);
        ...
        bt.setOnClickListener(v -> {
                    ...
                    builder = new AlertDialog.Builder(this);
                    builder.setCancelable(true);
                    builder.setPositiveButton("Yes", (dialog, which) -> {dialog.dismiss(); finish();
                    System.exit(0);});
                    builder.setNegativeButton("no",(dialog,which)-> dialog.dismiss());
                    builder.setMessage("Are you really quit!");
                    builder.show();
    ...
    private void resetBoard() {
        ...
        player1Turn = r1 % 2 == 0;
        if(player1Turn){
            Toast.makeText(this, "Player X got chance", Toast.LENGTH_SHORT).show();
        }
        else {
            Toast.makeText(this, "Player O got chance", Toast.LENGTH_SHORT).show();
        }
    }
